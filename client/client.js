...
var global = {
    ...
    scrollbar: (function () {
        ...
        div.addEventListener('click', function () {
            clicked = !clicked;
            for (let i = 0; i < slide.length; i++) slide[i].style.top = `${-265 * clicked}px`;
            if (clicked) {
                div.innerHTML = '<h3 class="nopadding">close options/help 🡶</h3>';
            } else {
                div.innerHTML = '<h3 class="nopadding">view options/help 🡲</h3>';
            };
        });
    })()
};
    ...
    return {
        ...
        get: () => {
            ...
            display =
                timediff < speed
                    ? oldvalue +
                    (value - oldvalue) * Math.pow(timediff / speed, 1 / sharpness)
                    : value;
            return display;
        },
    };
...
const socketInit = (() => {
    ...
    */
    ...
    return (port) => {
        let socket = new WebSocket("ws://" + global.server);
        // Set up our socket
        ...
        return {
            draw: (
                ...
                ctx.drawImage(
                    tctx.canvas,
                    Math.round(x - xx),
                    Math.round(y - yy * (1.05 + !center * 0.45))
                );
                ctx.restore();
            },
            remove: () => {
...
const drawEntity = (() => {
    ...
    return (
        ...
        setColor(
            context,
            mixColors(color.grey, render.status.getColor(), render.status.getBlend())
        );
